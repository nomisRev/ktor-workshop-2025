...
private fun Application.configureJwt() {
    authentication {
        jwt("jwt") {
            ...
            validate { credential ->
                if (credential.payload.expiresAt.time <= System.currentTimeMillis()) return@validate null
    ...
    routing {
        authenticate("jwt") {
            get("/token") {
                val email =
                    call.principal<JWTPrincipal>()?.payload?.claims["email"] ?: return@get call.respond(Unauthorized)
                call.respond("Hi, $email")
            }
        }
    }
}
